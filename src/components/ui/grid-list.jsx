...
import { ArrowRight, ArrowUpRight, CheckCircle, ContactRound, Hand, Server, UserCircle } from 'lucide-react'
    ...
    <div className="flex items-center justify-center p-8">
      <div className="overflow-hidden rounded-[1rem] bg-muted shadow-sm grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 sm:gap-0.5 space-y-0.5 sm:space-y-0 p-0.5">
        {actions.map((action) => (
          <UiCard
            key={action.title}
            className={cn('group relative rounded-xl border-0 bg-card p-0 focus-within:ring-2 focus-within:ring-ring focus-within:ring-inset')}
          >
