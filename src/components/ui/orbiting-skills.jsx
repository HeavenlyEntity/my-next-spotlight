...
const OrbitingSkill = memo(({ config, angle }) => {
  ...
  return (
    ...
    >
      <div
        className={`
          backdrop-blur-xs relative flex h-full w-full cursor-pointer
          items-center justify-center rounded-full bg-gray-800/90
      ...
      >
        ...
        {isHovered && (
          <div className="backdrop-blur-xs pointer-events-none absolute -bottom-8 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-sm bg-gray-900/95 px-2 py-1 text-xs text-white">
            {label}
          </div>
        )}
      </div>
...
export default function OrbitingSkills({ showBackdrop = true }) {
  const [time, setTime] = useState(0)
  const [isPaused, setIsPaused] = useState(false)
  useEffect(() => {
    if (isPaused) return
    // Skills stay visible at their initial positions; only the orbit stops.
  ...
  return (
    <section className="!contents w-full items-center justify-center overflow-hidden">
      {showBackdrop && (
      ...
      <div
        className="align-self-center relative flex h-[calc(100vw-40px)] w-[calc(100vw-40px)] items-center justify-center justify-self-end md:h-[450px] md:w-[450px]"
        onMouseEnter={() => setIsPaused(true)}
        onMouseLeave={() => setIsPaused(false)}
      >
        {}
        <div className="bg-linear-to-br relative z-10 flex h-20 w-20 items-center justify-center rounded-full from-gray-700 to-gray-900 shadow-2xl">
