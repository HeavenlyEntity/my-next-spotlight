...
/* Ported verbatim from the "minimal" landing template
   ...
   tokens. Cards sit on a circle and the whole ring rotates; each card is
   counter-rotated so it always faces outward. */
  ...
  useEffect(() => {
    ...
    const animate = (time) => {
      if (
        ...
        /* Cap deltaTime to prevent huge jumps when returning from an
           inactive tab. */
            ...
            return (
              <motion.div
                ...
                className={cn(
                  ...
                  'border-[var(--amw-line)] border',
                  'bg-[var(--amw-card)] text-zinc-900 dark:text-zinc-100',
                  cardClassName
                )}
                style={{
                  width: `${cardWidth}px`,
