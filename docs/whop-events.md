...
| `begin_checkout` | custom | `BuyButton` `onSubmit`, before the redirect to Creem | none (each press is an attempt) | `value`, `content_*` |
...
- **Never fire `purchase` for a sale Whop processed.** Creem is not Whop, so
  ours are the "own checkout" case Whop documents. If a Whop checkout is ever
  added, do not report it -- Whop already does.
